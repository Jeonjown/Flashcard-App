...
const Flashcard = () => {
  ...
  // Fetch flashcards when component mounts or deckId changes
  useEffect(() => {
    const fetchFlashcards = async () => {
      try {
        ...
        await dispatch({ type: "SET_FLASHCARDS", payload: data.flashcards });
      } catch (error) {
    ...
    fetchFlashcards();
  }, [deckId, dispatch]); // Add deckId and dispatch to the dependency array
    ...
    <>
      ...
      {flashcards && flashcards.length > 0 ? (
        flashcards.map((flashcard) => (
          <div
            key={flashcard._id}
            className="mb-2 flex items-center bg-white p-4 shadow-sm"
          >
